...
// Get approved teachers for adults section
$teacherService = new TeacherService($conn);
$teachers = $teacherService->getTeachersByCategory('adults', []);
...
<head>
    ...
    <style>
        ...
        .plan-card.best-value {
            ...
            box-shadow: 0 15px 50px rgba(0, 64, 128, 0.3);
        }
        .plan-card.best-value::before {
            ...
            background: linear-gradient(135deg, #004080, #0b6cf5);
            color: white;
            padding: 6px 24px;
            border-radius: 20px;
            font-size: 0.85rem;
...
<body class="track-adults">
    ...
    <section class="teachers-section" style="background: white; padding: 60px 20px; margin-top: 40px;">
        <div style="max-width: 1200px; margin: 0 auto;">
            <h2 style="text-align: center; font-size: 2.5rem; margin-bottom: 15px; color: #004080; font-weight: 700;">
            ...
            <p style="text-align: center; font-size: 1.1rem; color: #666; margin-bottom: 40px; max-width: 700px; margin-left: auto; margin-right: auto;">
                Meet our experienced teachers approved for Adults classes. Click on any teacher to view their profile and select a plan.
            </p>
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 30px; margin-top: 40px;">
                <?php foreach ($teachers as $teacher): ?>
                    <div class="teacher-card" style="background: white; border-radius: 16px; padding: 30px; box-shadow: 0 4px 20px rgba(0, 64, 128, 0.15); transition: all 0.3s ease; text-align: center; cursor: pointer;" onclick="window.location.href='teacher-profile.php?id=<?php echo intval($teacher['id']); ?>'">
                        <img src="<?php echo htmlspecialchars($teacher['profile_pic'] ?? getAssetPath('images/placeholder-teacher.svg')); ?>" 
                             ...
                             style="width: 120px; height: 120px; border-radius: 50%; object-fit: cover; margin: 0 auto 20px; border: 4px solid #004080;">
                        <h3 style="font-size: 1.3rem; font-weight: 700; color: #004080; margin-bottom: 10px;">
                        ...
                        <a href="teacher-profile.php?id=<?php echo intval($teacher['id']); ?>" 
                           class="btn-primary" 
                           style="display: inline-block; padding: 12px 24px; border-radius: 25px; text-decoration: none; font-weight: 600; background: linear-gradient(135deg, #004080, #0b6cf5); color: white; transition: transform 0.2s;">
                            View Profile
                        </a>
                    </div>
                <?php endforeach; ?>
